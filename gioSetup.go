...
func main() {
    ...
    numarg := len(os.Args)
    flags:=[]string{"dbg","entries"}

    // default file

    useStr := "./gioSetup [/entries=n] [/dbg]"
    helpStr := "program that test giouring setup"
...
//        fmt.Printf("directio: %t\n", directio)
	...
	if err := UringSetupTest(numEntries); err != nil {log.Fatalf("error -- UringSetupTest: %v",err)}
